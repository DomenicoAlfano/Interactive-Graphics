...
FSNRender = Klass({
    ...
    create_text_mesh: function (fs_node, f, s) {
        ...
        if (fs_node.is_file) {
            txt.setScale(s * (5 / Math.max(320, txt.realWidth)));
    ...
    do_the_hitler_thingy: function () {
        ...
        body.childNodes[SUPER_RIGHT_ARM].setScale(0.3, 2, 1.0);
        body.childNodes[RIGHT_ARM].setScale(0.0, 0.0, 0.0);

        body.childNodes[SUPER_RIGHT_ARM].setAxis(1, 0, 1);
        body.childNodes[SUPER_RIGHT_ARM].setAngle(3.14 * 1/ 4);
        //body.childNodes[SUPER_RIGHT_ARM].setAxis(1, 0, 0);
        //body.childNodes[SUPER_RIGHT_ARM].setAngle(3.14 * 1 / 4);

        body.childNodes[SUPER_RIGHT_ARM].setPosition(-1.2, 1.0, 0);
        self = this;

        setTimeout(function(){
            body.childNodes[RIGHT_ARM].setScale(0.3, 1, 0.5);
            body.childNodes[SUPER_RIGHT_ARM].setScale(0.0, 0.0, 0.0);
        }, 500);
    },
    sceneUpdate: function () {
        if (this.avatar_moved) {
            if (!this.avatar) {
                ...
                var super_armr;
                super_armr = new Magi.Cube().setScale(0.0,0.0,0.0);//(0.3, 2, 0.3);
                ...
                Magi.Texture.load("arm.jpg", function (tex) {
                    ...
                    super_armr.material.textures.EmitTex = tex;
                });
			...
			console.log(other_thing.entry.name, "is_inside")
                    }
                } else if (is_falling) {
                    // it's a folder
    ...
    is_inside: function (avatar, other_thing) {
        ...
        if(avatar.absolutePosition[2] < -300 && other_thing_scale[0] * other_thing_scale[1] * other_thing_scale[2] == 0){
            other_thing_scale[0] = 0.33;
            other_thing_scale[1] = 0.33;
            other_thing_scale[2] = 0.33;
        }
        ...
        var cmaxz = other_thing_model.absolutePosition[2] + (other_thing_model.scaling[2] * other_thing_scale[2]) / 2;
        var cminz = other_thing_model.absolutePosition[2] - (other_thing_model.scaling[2] * other_thing_scale[2]) / 2;
    ...
    is_inside_cl: function (avatar, connector) {
        ...
        if (avatar.position[2] < to_platform[2])
		return false;

 	//if(avatar.position[2] == from_platform[2]) alert('you won a cake!')
 	if(avatar.position[2] > from_platform[2])
		 return false;
