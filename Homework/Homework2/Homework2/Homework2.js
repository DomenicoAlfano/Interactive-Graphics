...
function middleArm4() {
    var s = scale4(MIDDLE_ARM_WIDHT, MIDDLE_ARM_HEIGHT, MIDDLE_ARM_WIDHT);
    var instanceMatrix = mult(translate( -0.5, 0.45 * MIDDLE_ARM_HEIGHT, 9.5 ),s);
    var t = mult(modelViewMatrix, instanceMatrix);
    gl.uniformMatrix4fv( modelViewMatrixLoc,  false, flatten(t) );
    gl.drawArrays( gl.TRIANGLES, 0, NumVertices );
}
...
{
    var s = scale4(LOWER_ARM_WIDTH, LOWER_ARM_HEIGHT, LOWER_ARM_WIDTH);
    var instanceMatrix = mult(translate( -0.5, 0.5 * LOWER_ARM_HEIGHT, 9.5 ), s);
    var t = mult(modelViewMatrix, instanceMatrix);
    gl.uniformMatrix4fv( modelViewMatrixLoc,  false, flatten(t) );
    gl.drawArrays( gl.TRIANGLES, 0, NumVertices );
}
...
var render1=function(){
    ...
    modelViewMatrix = mult(modelViewMatrix, translate(0.0, BASE_HEIGHT, 0.0));
    modelViewMatrix  = mult(modelViewMatrix, rotate(-70, -35, 45, 1 ) );
    modelViewMatrix = mult(modelViewMatrix, rotate(theta[LowerArm]/2, 0, 0, 1 ));
    lowerArm4();

    modelViewMatrix = mult(modelViewMatrix, translate(0.0, LOWER_ARM_HEIGHT, 0.0));
    modelViewMatrix = mult(modelViewMatrix, rotate(theta[MiddleArm]/2, 0, 0, 1 ));
...
function CloseHand(decade){
    console.log("CloseHand", decade)
    theta[1] = decade;
    theta[2] = decade;
    theta[3] = decade;
    if(decade < 90){
        setTimeout(function(){CloseHand(decade + 1)}, 40);
    }
}
